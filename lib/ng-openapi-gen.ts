...
export class NgOpenApiGen {
    ...
    generate(): void {
        ...
        try {
            // Generate each model
            const models = this.sortModels([...this.models.values()]);
            for (const model of models) {
                this.write('model', model, model.fileName, 'models');
            }

            // Generate each service
            const services = this.sortServices([...this.services.values()]);
            ...
            const modelImports = this.globals.modelIndexFile ||
                                 this.options.indexFile
                                 ? models.map(m => new Import(m.name,
                                                              './models',
                                                              m.options))
                                 : null;
            if (this.globals.modelIndexFile) {
                this.write('modelIndex',
                           {...general, modelImports},
                           this.globals.modelIndexFile);
            }
            if (this.globals.serviceIndexFile) {
                this.write('serviceIndex',
                           general,
                           this.globals.serviceIndexFile);
            }
    ...
    private sortModels(models: Model[]): Model[] {
        ...
        for (const model of models) {
            if (model.properties) {
                model.properties = model.properties.sort((a: Property,
                                                          b: Property) => a.name.localeCompare(
                    b.name));
            }
            if (model.imports) {
    ...
    private ignoreUnusedModels() {
        ...
        usedNames.clear();
        referencedModels.forEach(name => this.collectDependencies(name,
                                                                  usedNames));
    ...
    private collectDependencies(name: string, usedNames: Set<string>) {
        ...
        this.allReferencedNames(model.schema)
            .forEach(n => this.collectDependencies(n, usedNames));
    }
    ...
    private allReferencedNames(schema: SchemaObject | ReferenceObject | undefined): string[] {
        ...
        const result: string[] = [];
        (schema.allOf || []).forEach(s => Array.prototype.push.apply(result,
                                                                     this.allReferencedNames(
                                                                         s)));
        (schema.anyOf || []).forEach(s => Array.prototype.push.apply(result,
                                                                     this.allReferencedNames(
                                                                         s)));
        (schema.oneOf || []).forEach(s => Array.prototype.push.apply(result,
                                                                     this.allReferencedNames(
                                                                         s)));
        ...
        if (schema.items) {
            Array.prototype.push.apply(result,
                                       this.allReferencedNames(schema.items));
        }
        return result;
    }
}
...
export async function runNgOpenApiGen() {
    ...
    try {
        const openApi = await refParser.bundle(input, {
            ...
            resolve:     {
                http: {
                          timeout: options.fetchTimeout == null
                                   ? 20000
