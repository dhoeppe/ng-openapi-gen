...
it('Api', done => {
    ...
    if (api) {
        ...
        parser.parseSource(ts).then(ast => {
            ...
            expect(ast.imports.find(i => i.libraryName ===
                                         '../models/referenced-in-service-one-of-1'))
                .withContext('ref1 import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName ===
                                         '../models/referenced-in-service-one-of-2'))
                .withContext('ref2 import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName ===
                                         '../models/a/b/ref-object'))
                .withContext('a.b.RefObject import')
                .toBeDefined();
            // But not referenced-in-one-of, as it is nested within an object schema
            expect(ast.imports.find(i => i.libraryName ===
                                         '../models/referenced-in-one-of'))
                .withContext('ref import')
                .toBeUndefined();
            done();
        });
    }
...
describe('Generation tests using all-types.json', () => {
    ...
    it('NullableObject model', done => {
        ...
        parser.parseSource(ts).then(ast => {
            expect(ast.imports.length).toBe(0);
            expect(ast.declarations.length).toBe(1);
            expect(ast.declarations[0])
                .toEqual(jasmine.any(InterfaceDeclaration));
    ...
    it('x.y.RefObject model', done => {
        ...
        parser.parseSource(ts).then(ast => {
            ...
            expect(ast.imports.find(i => i.libraryName ===
                                         '../../a/b/ref-object'))
                .withContext('a/b/ref-object import')
                .toBeDefined();
            expect(ast.declarations.length).toBe(1);
            expect(ast.declarations[0])
                .toEqual(jasmine.any(InterfaceDeclaration));
    ...
    it('union model', done => {
        ...
        const parser = new TypescriptParser();
        parser.parseSource(ts).then(ast => {
            expect(ast.imports.find(i => i.libraryName === './ref-enum'))
                .withContext('ref-enum import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName === './container'))
                .withContext('container import')
                .toBeDefined();
            expect(ast.declarations.length).toBe(1);
            expect(ast.declarations[0])
                .toEqual(jasmine.any(TypeAliasDeclaration));
            const decl = ast.declarations[0] as TypeAliasDeclaration;
            expect(decl.name).toBe('Union');
            const text = ts.substring(decl.start || 0, decl.end || ts.length);
            expect(text)
                .toBe(
                    'export type Union = { [key: string]: any } | RefEnum | RefIntEnum | RefNamedIntEnum | Container;');
            done();
        });
    });
    ...
    it('disjunct model', done => {
        ...
        parser.parseSource(ts).then(ast => {
            ...
            expect(ast.imports.find(i => i.libraryName ===
                                         './referenced-in-nullable-one-of'))
                .withContext('referenced-in-nullable-one-of import')
                .toBeDefined();
            expect(ast.declarations.length).toBe(1);
            expect(ast.declarations[0])
                .toEqual(jasmine.any(TypeAliasDeclaration));
            ...
            expect(text)
                .toBe(
                    'export type Disjunct = { \'ref\'?: ReferencedInNullableOneOf } | ABRefObject | XYRefObject | ReferencedInOneOf | EscapedProperties;');
            done();
        });
    });
    ...
    it('AdditionalProperties model', done => {
        ...
        parser.parseSource(ts).then(ast => {
            expect(ast.imports.length).toBe(1);
            expect(ast.imports.find(i => i.libraryName === './a/b/ref-object'))
                .withContext('a/b/ref-object import')
                .toBeDefined();
            expect(ast.declarations.length).toBe(1);
            expect(ast.declarations[0])
    ...
    it('Container model', done => {
        ...
        parser.parseSource(ts).then(ast => {
            ...
            expect(ast.imports.find(i => i.libraryName === './a/b/ref-object'))
                .withContext('a/b/ref-object import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName === './x/y/ref-object'))
                .withContext('x/y/ref-object import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName === './union'))
                .withContext('union import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName === './disjunct'))
                .withContext('disjunct import')
                .toBeDefined();
            expect(ast.imports.find(i => i.libraryName === './nullable-object'))
                .withContext('nullable-object import')
                .toBeDefined();
            ...
            function assertProperty(name: string,
                                    type: string,
                                    required = false) {
                ...
                if (prop) {
                    ...
                    expect(prop.isOptional)
                        .withContext(`${name} required`)
                        .toBe(!required);
                }
            }
    ...
    it('EscapedProperties model', done => {
        ...
        parser.parseSource(ts).then(ast => {
            ...
            expect(decl.properties.length).toBe(3);
            expect(decl.properties[0].name).toBe('=');
            expect(decl.properties[1].name).toBe('123');
